...
def cmd_status(args, root, project):
    ...
    all_modules = [m for b in plan.get("batches", []) for m in b.get("modules", [])]
    completed = [m for m in all_modules if m.get("state") == "completed"]
    pending = [m for m in all_modules if m.get("state") == "pending"]
    skipped = [m for m in all_modules if m.get("state") == "skipped"]

    print(f"\n📋 {plan.get('project', project)} · 计划进度")
    print(f"{'─'*40}")
    print(f"  ✅ 已完成: {len(completed)}")
    print(f"  ⏳ 待完成: {len(pending)}")
    print(f"  ⏭️ 已跳过: {len(skipped)}")
    print(f"  总计: {len(all_modules)}")
    print()
...
def cmd_validate(args, root, project):
    ...
    pending = [m for m in all_modules if m.get("state") == "pending"]
    if pending:
        print(f"❌ 仍有未完成模块: {', '.join(m['name'] for m in pending)}")
        sys.exit(1)
    print("✅ 所有模块已完成（或跳过），可以进入 validate-output")


def main():
    parser = argparse.ArgumentParser(description="Plan Tracker — 结构化进度管理")
