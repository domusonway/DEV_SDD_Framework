...
- 归属规则：框架文档默认 root `docs/`；项目执行文档默认 `<PROJECT_ROOT>/docs/`，任务/模块细节默认 `<PROJECT_ROOT>/docs/sub_docs/`；`PROJECT_PATH` 优先于 `projects/<PROJECT>`，workspace 子项目不得混写到 workspace 根。
